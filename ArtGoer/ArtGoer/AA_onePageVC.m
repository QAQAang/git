...
@interface AA_onePageVC ()<UITableViewDataSource, UITableViewDelegate>
...
@property (nonatomic, retain) UILabel *label_worksMaterial;
...
- (void)creatCell1{
    ...
    if (self.model.worksSize == nil || [self.model.worksSize  isEqual: @""]) {
        if (self.model.worksYears == nil || [self.model.worksYears  isEqual: @""]) {
            self.label_author.text = [NSString stringWithFormat:@"%@", self.model.author];
        }else{
            self.label_author.text = [NSString stringWithFormat:@"%@|%@", self.model.author, self.model.worksYears];
        }
    }else{
    self.label_author.text = [NSString stringWithFormat:@"%@|%@|%@", self.model.author, self.model.worksSize, self.model.worksYears];
    }
    [self.cell1 addSubview:self.label_author];
    self.label_worksMaterial = [[UILabel alloc] initWithFrame:CGRectMake(20, 75, [UIScreen mainScreen].bounds.size.width - 20, 20)];
    self.label_worksMaterial.text = self.model.worksMaterial;
    self.label_worksMaterial.font = [UIFont systemFontOfSize:15];
    [self.cell1 addSubview:self.label_worksMaterial];
...
- (void)getTaskData{
    [[[NSURLSession sharedSession] dataTaskWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://api.artgoer.cn:8084/artgoer/api/v1/user/126323/exhibit_second/work/%@?&workId=%@&token=07d48315-f9be-4d20-a2db-657f7284ad5c", self.model.id_AA, self.model.id_AA]] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
    ...
    NSString *url = [NSString stringWithFormat:@"http://api.artgoer.cn:8084/artgoer/api/v1/user/126323/exhibit/%@/work/%@/comments?token=07d48315-f9be-4d20-a2db-657f7284ad5c", self.exhibitID, self.model.id_AA];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if (self.scroll.contentOffset.y > 0) {
        ...
        frame.origin.y = frame.origin.y + (new.y - old.y) * 0.6;
    ...
    [self.delegate changeHeadView:new :old];
}
