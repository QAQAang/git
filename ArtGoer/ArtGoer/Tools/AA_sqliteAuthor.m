...
#import "FMDB.h"
#define SQLCREATE @"create table if not exists author (id integer primary key , name text , pic text , adress text );"
#define SQLSELECTALL @"select * from author"
#define SQLINSERT @"insert into author (name , pic , adress ) values ('%@' , '%@' , '%@' )", name, pic , adress
#define SQLDELETE @"delete from author where name = %@",title
#define SQLSELECTbyNANE @"select * from author where name = '%@'", title
#define SQLDELETEALL @"delete from author"

static FMDatabase *db;
static NSString *path;

@implementation AA_sqliteAuthor
/**只在第一次使用时会调用的方法 */

+ (void)initialize{
    path = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject];
    NSString *filePath = [path stringByAppendingPathComponent:@"author.sqlite"];
...
+ (BOOL)insertUserWithTitle:(NSString *)title PicUrl:(NSString *)picUrl Url:(NSString *)url{
    NSString *name = title;
    if ([AA_sqliteAuthor selectUserByTitle:name].count == 0) {
        [db open];
        NSString *name = title;
        NSString *pic = picUrl;
        NSString *adress = url;
